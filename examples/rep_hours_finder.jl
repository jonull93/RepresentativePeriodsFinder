...
opt = optimizer_with_attributes(
    Gurobi.Optimizer, "time_limit" => 60.0, "output_flag" => true
);
...
for (ts_name, ts) in RPF.get_set_of_time_series(pf)
    p = create_duration_curve(
        pf, ts_name; line=:solid, marker=:none, original_discretised=true, aggregated_discretised=true
    )
    display(p)
    # for i in 1:2
    #     p.series_list[i].plotattributes[:seriestype] = :none
    # end
